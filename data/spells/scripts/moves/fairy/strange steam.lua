...
function onCastSpell(creature, variant)
	...
	local function sendDist(posi1, posi2, delay)
		if posi1 and posi2 and Creature(creature) then
			addEvent(doSendDistanceShoot, delay, posi1, posi2, 107)
		end
	end
	local function sendDano(creature, pos, delay, eff)
		creature = Creature(creature)
		if pos and Creature(creature) then
			addEvent(doAreaCombatHealth, delay, creature.uid, COMBAT_FAIRYDAMAGE, pos, 0, damage, damage, eff)
		end
	end
	if Creature(creature) then
		for j = 1, 4 do
			for i = 1, 6 do
				...
				addEvent(sendDano, 350, creature.uid, pos1[j][i], i*200, 410)
				addEvent(sendDist, 350, pos2[j][i], pos2[j][i+1], i*210)
				addEvent(sendDano, 350, creature.uid, pos2[j][i], i*200, 410)
				addEvent(sendDist, 800, pos3[j][i], pos3[j][i+1], i*210)
				addEvent(sendDano, 800, creature.uid, pos3[j][i], i*200, 410)
				addEvent(sendDist, 800, pos4[j][i], pos4[j][i+1], i*210)
				addEvent(sendDano, 800, creature.uid, pos4[j][i], i*200, 410)
			end
		end
	end
	return true
