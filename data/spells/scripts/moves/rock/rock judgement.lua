...
local function doDoom(creature, target, times)
	...
	doSendDistanceShoot(Position(pos), target:getPosition(), 164)
	if math.fmod(times, 4) == 0 then
		addEvent(doTargetCombatHealth, times * 100, creature.uid, target.uid, COMBAT_ROCKDAMAGE, damage, damage, 0)
