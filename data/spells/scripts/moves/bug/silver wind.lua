...
function onCastSpell(creature, variant)
	...
	for i = 1, 100 do
		...
		if math.random(1, 2) == 1 then
			addEvent(doSendMagicEffect, i * 10, place, 692)
		end
	end
	pos:sendMagicEffect(1008)
	addEvent(doAreaCombatHealth, 250, creature.uid, COMBAT_BUGDAMAGE, creature:getPosition(), area, damage, damage, 0)
	addEvent(doAreaCombatHealth, 500, creature.uid, COMBAT_BUGDAMAGE, creature:getPosition(), area, damage, damage, 0)
