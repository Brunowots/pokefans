function onCastSpell(creature, variant)
	...
	if Creature(creature):getMaster() then
		if creature:getCondition(CONDITION_MANASHIELD) ~= nil then
			...
			for i = 1, 10 do
				if creature:getMaster():getUsingBall():getSpecialAttribute("sketch"..tostring(i)) and creature:getMaster():getUsingBall():getSpecialAttribute("sketch"..tostring(i)) ~= 0 and creature:getMaster():getUsingBall():getSpecialAttribute("sketch"..tostring(i)):split("|")[1] == moves[9].name then
					creature:getMaster():sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Voce nao pode copiar um ataque que voce ja conhece.")
					return false
				end
			end
			local stg = "" .. moves[9].name .. "|" .. moves[9].speed .. "|" .. moves[9].level .."|" .. moves[9].isTarget .. "|" .. moves[9].range
			creature:getMaster():getUsingBall():setSpecialAttribute("sketch9", stg)
