function onCastSpell(creature, variant)
	...
	if Creature(creature):getMaster() then
		if creature:getCondition(CONDITION_MANASHIELD) ~= nil then
			...
			for i = 1, 10 do
				if creature:getMaster():getUsingBall():getSpecialAttribute("sketch"..tostring(i)) and creature:getMaster():getUsingBall():getSpecialAttribute("sketch"..tostring(i)) ~= 0 and creature:getMaster():getUsingBall():getSpecialAttribute("sketch"..tostring(i)):split("|")[1] == moves[3].name then
					creature:getMaster():sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Voce nao pode copiar um ataque que voce ja conhece.")
					return false
				end
			end
			local stg = "" .. moves[3].name .. "|" .. moves[3].speed .. "|" .. moves[3].level .."|" .. moves[3].isTarget .. "|" .. moves[3].range
			creature:getMaster():getUsingBall():setSpecialAttribute("sketch3", stg)
