...
local function doDoom(creature, target, times)
	...
	doSendDistanceShoot(Position(pos), target:getPosition(), 143)
	if math.fmod(times, 4) == 0 then
		addEvent(doTargetCombatHealth, times * 100, creature.uid, target.uid, COMBAT_DARKDAMAGE, damage, damage, 0)
