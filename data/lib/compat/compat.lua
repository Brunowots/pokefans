...
-- COMBAT_EARTHDAMAGE = COMBAT_POISONDAMAGE
...
function doPlayerFeed(cid, food, health) local p = Player(cid) return p ~= nil and p:feed(food, health) or false end
...
function getGlobalStorageValue(key)
    local query = db.storeQuery("SELECT `value` FROM `znote_global_storage` WHERE `key` = '" .. key .. "';")
    if query ~= false then
...
function setGlobalStorageValue(key, value)
    db.query("INSERT INTO `znote_global_storage` (`key`, `value`) VALUES ('".. key .."', '".. value .."') ON DUPLICATE KEY UPDATE `value` = '".. value .."';")
    return true
end
