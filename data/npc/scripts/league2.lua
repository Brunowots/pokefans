...
function creatureSayCallback(cid, type, msg)
	...
	elseif (msgcontains(msg, 'liga') or msgcontains(msg, 'entrar')) and getBadgeQuantity(Player(cid)) ~= 8 then
		...
		npcHandler:releaseFocus(cid)
	elseif (msgcontains(msg, 'liga') or msgcontains(msg, 'entrar')) and getBadgeQuantity(Player(cid)) == 8 then
		if not areaHasPlayer(Position(246, 870, 5), 2, 2) and not areaHasPlayer(Position(246, 870, 4), 2, 2) and not areaHasPlayer(Position(246, 870, 3), 2, 2) and not areaHasPlayer(Position(246, 870, 2), 2, 2) and not areaHasPlayer(Position(246, 870, 1), 2, 2) then
			Player(cid):teleportTo(Position(245, 872, 5))
			npcHandler:releaseFocus(cid)
		else
